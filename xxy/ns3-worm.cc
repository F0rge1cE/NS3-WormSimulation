...
void Worm::Listen(ns3::Ptr<ns3::Socket> socket){
  while (socket->GetRxAvailable () > 0){
      ...
      std::cerr << "[" << m_name << "] receive package" << std::endl;
      if ((((uint32_t)m_infectionPort) == pktMsg) && (!m_infected))
        {
          m_infected = !m_infected;
          m_totalInfected++;
...
void Worm::StartInfectingNodes(){
      ...
      m_onoffSocket[0]->SetConnectCallback(ns3::MakeCallback(&Worm::ConnectionSucceeded, this),
